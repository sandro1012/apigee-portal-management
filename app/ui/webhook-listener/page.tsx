...
export default function WebhookListenerPage() {
  ...
  function validateForm(): string | null {
    ...
    if (!apisList.some(a => selectedApis[a.key])) return "Selecione ao menos uma API listener";
    ...
    if (authType === "Token" && (!oauthUrl.trim() || !key.trim() || !secret.trim()))
      return "Token: oauthUrl, key e secret são obrigatórios";
    if (authType === "TokenAPI" && (!oauthUrl.trim() || !login.trim() || !senha.trim()))
      return "TokenAPI: oauthUrl, login e senha são obrigatórios";
    if (
      authType === "TokenPassword" &&
      (!oauthUrl.trim() || !username.trim() || !password.trim() || !key.trim() || !secret.trim())
    )
      return "TokenPassword: oauthUrl, username, password, key e secret são obrigatórios";
    if (authType === "TokenFormUrlencoded" && (!oauthUrl.trim() || !key.trim() || !secret.trim()))
      return "TokenFormUrlencoded: oauthUrl, key, secret são obrigatórios";
    ...
    <main>
      ...
      <section className="card" style={{ display: "grid", gap: 8, maxWidth: 980, marginBottom: 12 }}>
        ...
        {(authType === "Token" || authType === "TokenFormUrlencoded") && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 8 }}>
            <label>URL OAuth2* <input value={oauthUrl} onChange={e => setOauthUrl(e.currentTarget.value)} placeholder="https://.../oauth/token" /></label>
