...
export default function SelectUI() {
  ...
  async function exportKvm() {
    ...
    setEditorStatus("JSON carregado. Edite e clique em Salvar para aplicar no Apigee.");
    setDiffPreview(null);
  }
  ...
  function downloadJson() {
    ...
    const file = `${org || "org"}_${env || "env"}_${kvm || "kvm"}_${stamp}.json`;
  ...
  async function showDiff() {
    try {
      ...
      setEditorStatus("Erro: " + (e.message || String(e)));
    }
  }
  ...
  async function saveEdit() {
    try {
      ...
      const parsed = JSON.parse(editorJson);
      if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.keyValueEntries)) {
        setEditorStatus("JSON inválido: esperado objeto com keyValueEntries[].");
        return;
      }
      setEditorStatus("Aplicando diffs (create/update/delete)...");
      ...
      if (!res.ok) {
        setEditorStatus("Falha: " + (j.error || res.statusText));
        return;
      }
  ...
  async function createNewKvm() {
    if (!org || !env || !newKvmName.trim()) {
      setCreateMsg("Preencha org/env/nome.");
      return;
    }
    ...
    try {
      ...
      } else if (editorJson.trim()) {
        data = JSON.parse(editorJson);
      } else {
        data = { keyValueEntries: [], nextPageToken: "" };
      }
      ...
      const j = await res.json();
      if (!res.ok) {
        setCreateMsg("Falha: " + (j.error || res.statusText));
        return;
      }
      ...
      setCreateMsg("Erro: " + (e.message || String(e)));
    }
  }
  ...
  return (
    <main>
      ...
      <div style={{display:'grid', gap:8, marginBottom:16, maxWidth:760, padding:12, border:'1px solid #ddd', borderRadius:8}}>
      ...
      <section style={{marginTop:24}}>
        ...
        <p style={{marginTop:-8, opacity:.75}}>Edite e use <b>Pré-visualizar diff</b> para ver o que será alterado. Depois clique <b>Salvar</b> para aplicar.</p>
        ...
        {diffPreview && (
          <div style={{marginTop:12, padding:12, border:'1px solid #ccc', borderRadius:8}}>
            ...
            <pre style={{marginTop:8, background:'#111', color:'#0f0', padding:12, borderRadius:8, maxHeight:240, overflow:'auto'}}>
{JSON.stringify(diffPreview, null, 2)}
      ...
      <section style={{marginTop:36}}>
        ...
        <div style={{display:'grid', gap:8, maxWidth:760, padding:12, border:'1px solid #ddd', borderRadius:8}}>
