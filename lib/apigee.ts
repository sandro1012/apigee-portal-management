...
/** Preferência de credenciais: cookie/env -> SA */
...
/** Exporta todas as entries (paginando) */
...
export async function createKvm(org: string, env: string, name: string, encrypted: boolean, bearer?: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`createKvm ${res.status} – ${txt}`);
  }
...
/** Atualiza uma entry existente (PUT) */
export async function updateEntry(org: string, env: string, map: string, entry: KvmEntry, bearer?: string) {
  const token = await ensureBearer(bearer);
  const namePath = `organizations/${org}/environments/${env}/keyvaluemaps/${map}/entries/${encodeURIComponent(entry.name)}`;
  const url = `${BASE}/v1/${namePath}`;
  const res = await fetch(url, {
    method: "PUT",
    headers: { Authorization: `Bearer ${token}`, "Content-Type":"application/json" },
    body: JSON.stringify(entry)
...
export async function deleteEntry(org: string, env: string, map: string, entryName: string, bearer?: string) {
  const token = await ensureBearer(bearer);
  const namePath = `organizations/${org}/environments/${env}/keyvaluemaps/${map}/entries/${encodeURIComponent(entryName)}`;
  const url = `${BASE}/v1/${namePath}`;
...
/** Concilia o KVM com o objeto alvo */
export async function updateKvmToMatch(org: string, env: string, map: string, target: {keyValueEntries: KvmEntry[]}, bearer?: string) {
  ...
  const toCreate: KvmEntry[] = [];
  const toUpdate: KvmEntry[] = [];
  ...
  // delete
  for (const [name] of curMap.entries()) {
    if (!tgtMap.has(name)) toDelete.push(name);
  }
